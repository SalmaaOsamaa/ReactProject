...
export default function Tasks(props){
    ...
    <div className={`taskDetailsForm${showEndAddress() ? "" : " d-none"}`}>
        ...
        <div className={`${done ? "" : "d-none"}${done1 ? "d-none" : ""}`}>
            ...
            <div className="d-flex justify-content-center mt-5 mb-3"><input type="button" className="btn btn-outline-primary px-4 py-2" value="Continue" onClick={()=>{showNext1()}}/></div>
        </div>
    </div>
    <div className={`taskDetailsForm${showTaskOptionsAndDetails() ? "" : " d-none"}`}>
        ...
        <div className={`${done1 ? "" : "d-none"}${done2 ? "d-none" : ""}`}>
            ...
            <div className="form-check form-check-inline">
                ...
                <label className="form-check-label">Small - Est. 1 hr</label>
                </div>
                <div className="form-check form-check-inline">
                <input className="form-check-input" type="radio" name="inlineRadioOptions"/>
                <label className="form-check-label">Medium - Est. 2-3 hrs</label>
                </div>
                <div className="form-check form-check-inline">
                <input className="form-check-input" type="radio" name="inlineRadioOptions"/>
                <label className="form-check-label">Large - Est. 4+ hrs</label>
            </div>
            <div className="d-flex justify-content-center mt-5 mb-3"><input type="button" className="btn btn-outline-primary px-4 py-2" value="Continue" onClick={()=>{showNext2()}}/></div>
        </div>
    </div>
    <div className={`taskDetailsForm${showTaskOptionsAndDetails() ? "" : " d-none"}`}>
        ...
        <div className={`${done2 ? "" : "d-none"}`}>
            ...
            <textarea className="form-control" rows="3" placeholder="Provide a summary of what you need done for your Tasker. Be sure to include details like the size of your space, any equipment/tools needed, and how to get in."/>
            <p>If you need two or more Taskers, please post additional tasks for each Tasker needed.</p>
            <div className="d-flex justify-content-center mt-5 mb-3"><input type="button" className="btn btn-outline-primary px-4 py-2" value="See Taskers & Prices"/></div>
        </div>
    </div>
    <div className={`taskDetailsForm${props.taskName === "Furniture Assembly" ? "" : " d-none"}`}>
        ...
        <div className={`${done ? "" : "d-none"}`}>
            ...
            <div className="form-check">
                ...
                <label className="form-check-label">IKEA furniture items only</label>
            </div>
            <div className="form-check">
                <input className="form-check-input" type="radio" name="flexRadioDefault"/>
                <label className="form-check-label">Other furniture items (non-IKEA)</label>
            </div>
            <div className="form-check">
                <input className="form-check-input" type="radio" name="flexRadioDefault"/>
                <label className="form-check-label">Both IKEA and non-IKEA furniture</label>
            </div>
            <div className="d-flex justify-content-center mt-5 mb-3"><input type="button" className="btn btn-outline-primary px-4 py-2" value="Continue"/></div>
        </div>
    </div>
    </>
}